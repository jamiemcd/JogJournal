...
@interface CFXJogViewController () <MKMapViewDelegate>
...
@implementation CFXJogViewController
...
- (void)viewDidLoad
{
	...
	[self createUI];
    [self updateUI];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(coreDataManagerNewLocationAddedToActiveJogHandler:) name:CFXCoreDataManagerNewLocationAddedToActiveJog object:[CFXCoreDataManager sharedManager]];
}
...
- (void)updateUI
{
    ...
    if (!jog)
    {
        jog = [CFXCoreDataManager sharedManager].activeJog;
    }
    if (jog)
    {
        if (jog.endDate)
        ...
        {
            CFXLocation *location = jog.locations[i];
...
- (void)zoomToBestZoomForCompletedJog
{
    ...
    if (count > 0)
    {
        ...
        {
            CFXLocation *location = self.jog.locations[i];
            if (i == 0)
            {
...
{
    [[CFXCoreDataManager sharedManager] startNewJog];
    [self updateUI];
}

- (void)completeButtonTouchHandler:(UIBarButtonItem *)barButtonItem
{
    [[CFXCoreDataManager sharedManager] completeActiveJog];
}
